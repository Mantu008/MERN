	...
	if ( !seed ) {
		...
		if ( documentIsHTML ) {
			...
			if ( nodeType !== 11 && ( match = rquickExpr.exec( selector ) ) ) {
				...
				// className selector
				} else if ( ( m = match[ 3 ] ) && context.getElementsByclassNameName ) {
...
function setDocument( node ) {
	...
	// Support: IE 9 - 11+, Edge 12 - 18+
	// IE/Edge don't support the :scope pseudo-className.
	...
	// className
	Expr.find.className = function( classNameName, context ) {
	...
	if ( !support.cssHas ) {
		...
		// pseudo-classNamees inside `:has()` (such as `:has(:contains("Foo"))`)
...
Expr = jQuery.expr = {
	...
	filter: {
		...
		className: function( classNameName ) {
			...
			return pattern ||
				( pattern = new RegExp( "(^|" + whitespace + ")" + classNameName +
					"(" + whitespace + "|$)" ) ) &&
		...
		PSEUDO: function( pseudo, argument ) {
			...
			// https://www.w3.org/TR/selectors/#pseudo-classNamees
...
jQuery.fn.extend( {
	addclassName: function( value ) {
		var classNameNames, cur, curValue, classNameName, i, finalValue;
		...
		if ( classNameNames.length ) {
			return this.each( function() {
				curValue = getclassName( this );
				cur = this.nodeType === 1 && ( " " + stripAndCollapse( curValue ) + " " );

				if ( cur ) {
					for ( i = 0; i < classNameNames.length; i++ ) {
						classNameName = classNameNames[ i ];
						if ( cur.indexOf( " " + classNameName + " " ) < 0 ) {
							cur += classNameName + " ";
						}
					}
					...
					if ( curValue !== finalValue ) {
						this.setAttribute( "className", finalValue );
					}
				}
			} );
		}
			...
			return this.each( function() {
				...
				if ( cur ) {
					for ( i = 0; i < classNameNames.length; i++ ) {
						...
						while ( cur.indexOf( " " + classNameName + " " ) > -1 ) {
							cur = cur.replace( " " + classNameName + " ", " " );
						}
					}
	...
	toggleclassName: function( value, stateVal ) {
		var classNameNames, classNameName, i, self,
			type = typeof value,
